...
public class HousingDataset {
    ...
    public static void main(String[] args) {
        ...
        Statistics dPO = new Statistics(
                hDList.stream().parallel().filter(hd -> hd.isChas()).mapToDouble(HousingDataset::getMedv).max()
                        .orElse(Double.NaN),
                hDList.stream().parallel().filter(hd -> hd.isChas()).mapToDouble(HousingDataset::getMedv).min()
                        .orElse(Double.NaN),
                hDList.stream().parallel().filter(hd -> hd.isChas()).mapToDouble(HousingDataset::getMedv)
